...
$varianty=array(
  ...
  'b'=>array('popis'=>'larpy a bonusy',    'db'=>'(t.id_typu=2 || t.id_typu=7)'),
  'c'=>array('popis'=>'RPG',               'db'=>'t.id_typu=4'),
  'd'=>array('popis'=>'přednášky a dílny', 'db'=>'(t.id_typu=3 || t.id_typu=5)'),
  );
...
$a=dbQuery('SELECT * FROM akce_seznam a
...
while($r=mysql_fetch_assoc($a))
{
  ...
  if($r['stav']==0) $tpl->parse('aktivity.aktivita.publikovat');
  if($r['stav']==4) $tpl->parse('aktivity.aktivita.aktivovat');
