...
$aktivity = Aktivita::zFiltru(['typ' => Typ::DRD, 'rok' => ROK]);
if(empty($aktivity)) {
  // když aktivity nejsou založeny, použít z minulého roku
  $aktivity = Aktivita::zFiltru(['typ' => Typ::DRD, 'rok' => ROK - 1]);
}
...
usort($organizatori, function($a, $b) { return strcasecmp($a->nick(), $b->nick()); });
