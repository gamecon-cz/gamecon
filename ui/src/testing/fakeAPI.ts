...
export const fetchTestovacíAktivityPřihlášen = async (rok: number): Promise<AktivitaPřihlášen[]> => {
  return (await fetchTestovacíAktivity(rok)).map(x => ({ id: x.id, mistnost: `místnost 5`, prihlasen: Math.random() < .3, vedu: Math.random() < .1, slevaNasobic: Math.random() < .1 ? undefined : Math.floor(Math.random() * 10) / 10 } as AktivitaPřihlášen));
};
