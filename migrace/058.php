...
$intoSqlValues          = function (array $values, mysqli $mysqli): string {
    return implode(
        ",\n", // ('foo','bar'),('baz','quz')
        array_map(
            function (array $row) use ($mysqli): string {
                return sprintf(
                    '(%s)', // ('foo','bar')
                    implode(
                        ',', // 'foo','bar'
                        array_map(
                            function (?string $value) use ($mysqli): string {
                                if ($value === null) {
                                    return 'NULL';
                                }
                                if (preg_match('~^[(].+[)]$~', $value)) {
                                ...
                                return sprintf("'%s'", mysqli_real_escape_string($mysqli, $value));
                            },
                            $row,
                        ),
                    ),
...
foreach ($subCategories as $parentCategoryName => $subCategoriesWithSameParent) {
    $subCategoriesWithSameParentForSql = array_map(
        function (array $subCategory) use ($parentCategoryName, $mysqli): array {
            ...
            $subCategory[1] = sprintf(
                ...
                mysqli_real_escape_string($mysqli, $parentCategoryName),
            ); // parent category ID
            return $subCategory;
        },
