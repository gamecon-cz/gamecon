...
**Třetí brána je ve frontendu (Preact).** I když backend interní aktivitu do `aktivitySkryte` pošle, program ji ještě filtruje v `ui/src/store/program/logic/aktivity.ts` (`filtrujAktivity`). Bez zapnutého admin tlačítka „Interní" (`zobrazInterni`/`filtrInterni`) se interní aktivity, ke kterým účastník nemá vztah, schovají jako clutter; ty, na které **je** přihlášen nebo které **vede** (`vedu`), zůstávají. Správná podmínka je `!aktivita.interni || !!aktivita.stavPrihlaseni || !!aktivita.vedu`. Historicky tu byla **obrácená negace** (`!aktivita.stavPrihlaseni`) → po posazení účastníka na interní aktivitu ta z programu (i z „můj program") **zmizela**, přitom ji účastník měl dál ve výpisu aktivit na tabu Uživatel. Pozor na `vedu`: „můj program" (`výběr.typ === "můj"`) bere aktivity přes `stavPrihlaseni !== null || vedu`, takže filtr interních musí `vedu` respektovat taky — jinak org přijde o svou neveřejnou technickou/brigádnickou aktivitu, na kterou není přihlášen. Opraveno 2026-07 (commit „Povolit zápis na skryté interní aktivity" byl backend; frontend fix následoval). **Bundle je verzovaný** (`web/soubory/ui/bundle.js` + `admin/files/ui/bundle.js`) — po změně `.ts` je nutný rebuild (`cd ui && vite build` + `vite build --outDir ../admin/files/ui`), jinak se změna neprojeví.

**Pozor na rozpojený přepínač (bug opravený 2026-07):** URL stav (`ProgramURLStav` v `logic/url.ts`) používá klíč **`zobrazInterni`**, ale `filtrujAktivity` čte **`filtrInterni`**. Most mezi nimi je jediné místo — `useFiltrAktivitNeboZeStavu` v `ui/src/store/program/selektory.ts`, které musí `filtrInterni: urlStav.zobrazInterni` explicitně namapovat. Původně tam byl jen `urlStav as FiltrAktivit` (jen přetypování), takže `filtrInterni` bylo **vždy `undefined`** → interní aktivity se schovávaly pro **všechny včetně adminů** a tlačítko „Interní" nedělalo nic. Symptom: nepublikované brigádnické/organizační aktivity zmizely z programu úplně a brigádnická sekce se jevila prázdná. Pokud sáhneš na názvy klíčů, tenhle bridge musí zůstat.
