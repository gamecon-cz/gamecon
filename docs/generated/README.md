...
- [sledovani-aktivit](sledovani-aktivit.md) — watchlist aktivity (mail při uvolnění místa); kde se zobrazuje „sledovat/zrušit", jediný trigger mailu v `odhlas()` podle rozdílu `volnoPro()` před/po odhlášení; tři chyby u genderově rozdělených aktivit (volno jen pro opačné pohlaví) — všechny opravené
...
- [prihlasovani-na-skryte-technicke-aktivity](prihlasovani-na-skryte-technicke-aktivity.md) — posazování na nepublikované (NOVA) technické/brigádnické aktivity přes příznak `Aktivita::INTERNI`; výjimka vázaná na typ (`jeInterni()`), ne na roli; dnes gated přes prezenčního admina (Pravo 103); proč to přestalo fungovat po migraci programu na Preact (ztracený `Program::INTERNI`); časová brána registrace
- [odkazy-do-archivnich-rocniku](odkazy-do-archivnich-rocniku.md) — proklik z ostrého adminu na tentýž objekt v archivním ročníku; archivy jedou na `ROK.gamecon.cz/admin` (ne `admin.ROK.gamecon.cz`), URL potřebuje tři parametry naráz (`gate` na bránu, `gcsso` na přihlášení, akční parametr), proč `gcsso` + `pracovni_uzivatel` na jedné URL funguje, a tři degradační stupně podle stáří ročníku (`pracovni_uzivatel` až od 2022, `gcsso` od 2014, živý admin od 2011, starší bez odkazu)
...
- [prihlaska-odeslani-objednavky](prihlaska-odeslani-objednavky.md) — co se stane po odeslání veřejné přihlášky; pořadí zpracování (ubytování musí předcházet jídlu kvůli snídaním v ceně hotelu), názvy POST polí a jejich nezávislost, počet kusů jako počet řádků; kde se hlídá vyprodání (`Shop::prodat()` vs. vlastní cesta ubytování) a rozpor, že pozastavený předmět zmizí z formuláře, ale ručně poskládaný POST ho koupí (touto cestou už nejspíš neplatí, viz dokument); gotchas pro testy (vlastní transakce, statická cache `Uzivatel`, konstanty termínů, XTemplate cache)
- [testovani-zabezpeceneho-api](testovani-zabezpeceneho-api.md) — jak se testují endpointy za firewallem `^/symfony/api` (JWT, role); testuje se přes `ApiTestCase::createClient()` s transakčním obalem (`AbstractDatabaseKernelTestCase`); past s `bootKernel()`/`createClient()`, která zahodí spojení držící transakci, klíč kolekce `member` místo `hydra:member`, `product_tag.created_at` bez defaultu; invariant „entita nikdy není veřejně dostupná, jen DTO" a jak ho hlídá `ApiSecurityTest`; a co jde převzít z `3brs/PiercingApp` (matice rolí s deny-listem jako doplňkem, oddělené testy 401/403) versus co je blokované
- [letosni-model-predmetu](letosni-model-predmetu.md) — „letošní placka/kostka“ je právě jeden produkt, ne všechny letos nabízené (v 2026 jsou čtyři placky s `model_rok = 2026`); dnešní heuristika o vítězi rozhoduje fakticky podle `id_predmetu`, chceme příznak na produktu s UNIQUE přes NULL — a proč to zrovna takhle jde vynutit
- [terminy-prodeje-a-ruseni](terminy-prodeje-a-ruseni.md) — po termínu jde merch zrušit, ale jídlo ne: do cateringu jde počet porcí, kdežto účastnických triček se bere zásoba navíc a doprodává se na pultu; historicky byl merch zamčený obousměrně („objednat **a měnit**“ v názvu nastavení); termín se ladí na třech místech (datum kategorie, `nabizet_do` produktu, zásoba) a **které kategorie sdílejí datum, se mezi ročníky mění**
- [tricka-velikosti-a-varianty](tricka-velikosti-a-varianty.md) — velikost trička je v legacy samostatný produkt (477 z 486 má velikost v `kod_predmetu`, ne spolehlivě v názvu); jak seskupovat (base kód + ročník = 112 skupin), že cena i kapacita jsou per-varianta, a proč vedle sized produktů existuje bezvelikostní řádek pro interní prodej na festivalu; taky že reporty nechtějí barvu, ale hodnost (org/vypravěč/účastník) a berou ji z `kod_predmetu` — dřív se hádala z barvy v názvu, jenže orgovská trička byla 2009 a 2010 oranžová
...
- [admin-objednavky-za-ucastnika](admin-objednavky-za-ucastnika.md) — poslední legacy zápisová cesta e-shopu: dvě admin obrazovky, kde obsluha objednává za účastníka; zápis ubytování už přes `/admin/customer-accommodation` hotový (a proč `ROLE_ADMIN` nemohl být bránou — ročníkové kódy rolí), co ještě chybí (čtecí endpoint, payload na POST, dva různé renderery, přeprodej kapacity, který legacy nabízelo jen šéfovi infopultu a nevynucovalo nikomu — nově hlídáno na serveru; JWT a identita pracovního uživatele; snídaně, které admin umí zrušit, ale ne vrátit) a že jídlo nemá v nové vrstvě zápisovou cestu vůbec
- [eshop-diferencni-overeni](eshop-diferencni-overeni.md) — než e-shop uvidí první člověk, porovnat ho bok po boku s legacy: dva worktree (`main` vs. eshop větev), obě DB ze stejného ostrého dumpu, scénáře přes celý životní cyklus ročníku od neotevřené registrace po zmrazená data po GC; porovnává se zápis do DB, chování GUI i adminní reporty — a proč to zelené testy nenahradí (schémata se liší o 23 migrací, takže `SELECT *` nestačí)
