...
- **Mrtvé reklamní embedy.** `<DIV class="rbl reklama">` s `document.write` iframe/img na **dávno mrtvé** ad domény (`adx1.adrenaline.cz`, u 2003 navíc `ad2.billboard.cz` v blocích `<!-- BBSTART -->`…`<!-- BBEND -->`). Prohlížeč na ně **visí až do timeoutu** → pomalé načtení. Má **každý** ročník Altar éry (2003–2008). Fix: smaž celý `rbl reklama` div (+ billboard BBSTART/BBEND blok) — regex `<DIV\s+CLASS="rbl reklama">.*?<!--\s*Reklama - konec\s*-->\s*</DIV>` (DOTALL). Reálně: 2003 (`9b3899734`), 2004 (`18ac802c6`), 2005 (`a7b680a96`).
