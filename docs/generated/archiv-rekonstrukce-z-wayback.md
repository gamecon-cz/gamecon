...
6. **Theme**:
   ...
   - **2006–2008 (Altar subdoména) mají vlastní theme** (`stylesheets/{content,layout,print}.css`, `layout/*.gif`, `img/org/*.gif`) — **NELZE recyklovat z 2011 styl1 ani z path-éry**, stáhni z Waybacku. **Pozadí-dlaždice viz landmine #10** (`layout.css` linkuje ~12 `back*.gif`/`gradient-*.gif`, které se snadno minou — vypadá to jako kompletní theme, ale bez nich je stránka bez kostkového pozadí a s nezarámovaným logem).
...
### 9. Mrtvé reklamy a CGI-generovaná navigace = pomalé/poskakující načítání (Altar éra)
...
### 10. Chybějící theme assety, které CSS linkuje přes `url()` → stránka bez pozadí/loga (vypadá rozbitě)
