...
// Archive 2022 production settings — committed to archive/2022.
//
...
// references them). The 2022-era DatabazoveNastaveni also reads
// DBM_NAME / DBM_PORT, so those are defined here as well.
...
if (!defined('DBM_SERV')) define('DBM_SERV', getenv('DBM_SERV')
    ?: DB_SERV);
if (!defined('DBM_PORT')) define('DBM_PORT', (int)(getenv('DBM_PORT')
    ?: DB_PORT));
...
// but the constants must exist or boot fails fatally. The 2022 era also
// reads DB_ANONYM_PORT, unlike 2023.
if (!defined('DB_ANONYM_SERV')) define('DB_ANONYM_SERV', getenv('DB_ANONYM_SERV')
    ?: DB_SERV);
if (!defined('DB_ANONYM_PORT')) define('DB_ANONYM_PORT', (int)(getenv('DB_ANONYM_PORT')
    ?: DB_PORT));
