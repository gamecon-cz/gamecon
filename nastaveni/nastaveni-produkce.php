...
// Archive 2021 production settings — committed to archive/2021.
//
...
// Archive 2021: single subdomain serves the whole year, with /admin and
// /cache/public as path aliases of the one Apache DocumentRoot. The
// admin.2021 / cache.2021 sub-subdomains are no longer published in DNS.
define('URL_WEBU', 'https://2021.gamecon.cz');               // absolutní url uživatelského webu
define('URL_ADMIN', 'https://2021.gamecon.cz/admin');        // absolutní url adminu (path-based)
define('URL_CACHE', 'https://2021.gamecon.cz/cache/public'); // url sdílených cachí (path-based)

// DB connection — env-driven (deploy script passes these via docker run -e).
// DB_SERV defaults to the docker0 bridge IP where the host MariaDB is
...
// the same as DB_USER. (2021-era code references DBM_* in some paths.)
...
// Production-only constants the legacy code references. Archives don't
// accept payments, run cron, or call external services — empty/default
// so the code is happy without exposing live credentials.
...
// SECRET_CRYPTO_KEY — used by Defuse\Crypto helpers in some paths. A frozen
// archive can reuse the standard dummy from nastaveni-local-default.php.
