<?php

return [
  ['dobrovolnik', 'dobrovoln{ík}', '
    Za to, že je GameCon tak skvělý, jak je, vděčíme nejen vypravěčům a organizátorům, ale i lidem, kteří pomáhají vybudovat jeho zázemí (příprava prostor, pomoc na infopultu apod.). Sháníme ochotné lidi z řad účastníků, kteří by nám pomohli. Více se dozvíš na <a href="chci-se-zapojit#dobrovolnici">chci pomoci</a>.
  '],
  ['vypravec', 'vypravěč{ka}', '
    Máš nápad na super aktivitu, která se hodí na GameCon? Vypravěč je pojem pro všechny kteří na GameConu nějakou aktivitu pořádají a bez nichž by žádný festival nebyl :) Napiš nám svůj nápad níž a ozveme se ti.
  '],
  ['organizator', 'organizátor{ka}', '
    Máš nápady, organizační talent a tah na branku? Do organizačního týmu nehledáme profíky, ale lidi, kteří mají chuť se učit nové věci, nebojí se práce, chtějí získat zkušenosti ve středně velké neziskovce a v neposlední řadě se chtějí podílet na tom, aby byl GameCon co nejlepší.<br>
    O tom jak GameCon děláme a koho hledáme se dozvíš v <a href="chci-se-zapojit#dobrovolnici">chci se zapojit</a>.
  '],
  ['partner', 'partner GameConu', '
    Máš na starosti značku, kterou chceš na festivalu pro bezmála 600 lidí propagovat? Podpořit GameCon finančně, mít na GameConu stánek, nebo s námi jinak spolupracovat? Napiš nám o tom níž nebo přímo hlavnímu hlavnímu organizátorovi na <a href="mailto:info@gamecon.cz">info@gamecon.cz</a> a ozve se ti zpátky s detaily.
  '],
];
