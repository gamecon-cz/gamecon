...
class Shop
{
    ...
    // typy předmětů
    public const TYP_PREDMET = 1;
    public const TYP_UBYTOVANI = 2;
    public const TYP_TRICKO = 3;
    public const TYP_JIDLO = 4;
    public const TYP_VSTUPNE = 5;
    public const TYP_PARCON = 6;
    public const TYP_PROPLACENI_BONUSU = 7;

    // stavy předmětů
    public const STAV_K_VOLNEMU_PRODEJI = 1;
    public const STAV_PRODEJ_S_KONTROLOU = 2;
    public const STAV_POZASTAVENY = 3;
    ...
    public function __construct(Uzivatel $u, $nastaveni = null) {
        $this->u = $u;
    ...
    ', [ROK, $this->u->id()]);
        ...
        while ($r = mysqli_fetch_assoc($o)) {
            $typ = $r['typ'];
            if ($typ == self::TYP_PROPLACENI_BONUSU) {
            ...
            $r['nabizet'] = $r['stav'] == self::STAV_K_VOLNEMU_PRODEJI; // v základu nabízet vše v stavu 1
            // rozlišení kam ukládat a jestli nabízet podle typu
            if ($typ == self::TYP_PREDMET) {
                $fronta = &$this->predmety[];
            } elseif ($typ == self::TYP_JIDLO) {
            ...
            } elseif ($typ == self::TYP_UBYTOVANI) {
                $r['nabizet'] = $r['nabizet'] || ($r['stav'] == self::STAV_POZASTAVENY && $this->nastaveni['ubytovaniBezZamku']);
                $fronta = &$this->ubytovani[];
            } elseif ($typ == self::TYP_TRICKO) {
                $smiModre = $this->u->maPravo(P_TRICKO_MODRA_BARVA);
                $smiCervene = $this->u->maPravo(P_TRICKO_CERVENA_BARVA);
                $r['nabizet'] = (
                    $r['nabizet']
                    || ($r['stav'] == self::STAV_PRODEJ_S_KONTROLOU && mb_stripos($r['nazev'], 'modré') !== false && $smiModre)
                    || ($r['stav'] == self::STAV_PRODEJ_S_KONTROLOU && mb_stripos($r['nazev'], 'červené') !== false && $smiCervene)
                );
                $fronta = &$this->tricka[];
            ...
            } elseif ($typ == self::TYP_VSTUPNE) {
                if (mb_stripos($r['nazev'], 'pozdě') !== false) {
                    $this->vstupnePozde = $r;
                } else {
                    $this->vstupne = $r;
                    $this->vstupneJeVcas = $r['stav'] != self::STAV_POZASTAVENY;
                }
            } else {
    ...
    public static function bezDne($nazev) {
    ...
    public function jidloHtml() {
        ...
        foreach ($druhy as $druh => $i) {
            foreach ($dny as $den => $i) {
                ...
                if ($jidlo && ($jidlo['nabizet'] || $jidlo['kusu_uzivatele'])) {
                    ...
                    $t->parse($jidlo['stav'] == self::STAV_POZASTAVENY && !$this->nastaveni['jidloBezZamku'] ? 'jidlo.druh.den.locked' : 'jidlo.druh.den.checkbox');
                }
        ...
        // info o pozastaveni
        if (!$dny || $jidlo['stav'] == self::STAV_POZASTAVENY) {
    ...
    public function predmetyHtml() {
        ...
        // předměty
        if (current($this->predmety)['stav'] == self::STAV_POZASTAVENY) {
        ...
        // trička
        $zamceno = false;
        if (current($this->tricka)['stav'] == self::STAV_POZASTAVENY) {
    ...
    public function zpracujPredmety() {
        if (isset($_POST[$this->klicP]) && isset($_POST[$this->klicT])) {
            ...
            $stare = [];
            $o = dbQuery('SELECT id_predmetu FROM shop_nakupy JOIN shop_predmety USING(id_predmetu) WHERE id_uzivatele=' . $this->u->id() . ' AND rok=' . ROK . ' AND typ IN(' . self::TYP_PREDMET . ',' . self::TYP_TRICKO . ') ORDER BY id_predmetu');
            while ($r = mysqli_fetch_assoc($o))
    ...
    public function zpracujVstupne() {
        ...
        $vstupneVcas = $this->vstupneJeVcas
            ? $castka
            : $this->vstupne['sum_cena_nakupni'];
        $vstupnePozde = $this->vstupneJeVcas
            ? 0
            : max(0, $castka - $this->vstupne['sum_cena_nakupni']);
    ...
    static function zrusObjednavkyPro($uzivatele, $typ) {
        $povoleneTypy = [self::TYP_PREDMET, self::TYP_UBYTOVANI, self::TYP_TRICKO, self::TYP_JIDLO];
        if (!in_array($typ, $povoleneTypy)) {
            throw new Exception('Tento typ objednávek není možné hromadně zrušit');
        }

        $ids = array_map(static function (Uzivatel $u) {
            return $u->id();
        }, $uzivatele);
    ...
    private function cenaTricka() {
        $ceny = array_map(function ($t) {
            return $t['cena_aktualni'];
        }, $this->tricka);
        return max($ceny);
    }
...
LIMIT 1
SQL
            , [self::TYP_PROPLACENI_BONUSU]
        );
        if (!$idPredmetuPrevodBonsuNaPenize) {
            throw new RuntimeException(sprintf('Chybi virtualni "predmet" pro prevod bonusu na penize s typem %d', self::TYP_PROPLACENI_BONUSU));
        }
        dbQuery(<<<SQL
