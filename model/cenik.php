...
class Cenik
{
    ...
    public function shop($r): float {
        ...
        } elseif ($typ == Shop::TYP_TRICKO && mb_stripos($r['nazev'], 'modré') !== false && $this->modrychTricekZdarma > 0) {
            $cena = 0;
            $this->modrychTricekZdarma--;
        } elseif ($typ == Shop::TYP_TRICKO && $this->jakychkoliTricekZdarma > 0) {
            $cena = 0;
            $this->jakychkoliTricekZdarma--;
        } elseif ($typ == Shop::TYP_UBYTOVANI && $this->u->maPravo(P_UBYTOVANI_ZDARMA)) {
            $cena = 0;
        } elseif ($typ == Shop::TYP_UBYTOVANI && $r['ubytovani_den'] == 0 && $this->u->maPravo(P_UBYTOVANI_STREDA_ZDARMA)) {
            $cena = 0;
        } elseif ($typ == Shop::TYP_UBYTOVANI && $r['ubytovani_den'] == 4 && $this->u->maPravo(P_UBYTOVANI_NEDELE_ZDARMA)) {
            $cena = 0;
        } elseif ($typ == Shop::TYP_JIDLO) {
