...
namespace RectorPrefix202603\React\Dns\Query;

use RectorPrefix202603\React\Dns\Model\Message;
use RectorPrefix202603\React\Dns\Protocol\BinaryDumper;
use RectorPrefix202603\React\Dns\Protocol\Parser;
use RectorPrefix202603\React\EventLoop\Loop;
use RectorPrefix202603\React\EventLoop\LoopInterface;
use RectorPrefix202603\React\Promise\Deferred;
/**
...
final class UdpTransportExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        if (isset($queryData[$this->maxPacketSize])) {
            return \RectorPrefix202603\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Query too large for UDP transport', \defined('SOCKET_EMSGSIZE') ? \SOCKET_EMSGSIZE : 90));
        }
        ...
        if ($socket === \false) {
            return \RectorPrefix202603\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to connect to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        ...
        if ($written !== \strlen($queryData)) {
            return \RectorPrefix202603\React\Promise\reject(new \RuntimeException('DNS query for ' . $query->describe() . ' failed: Unable to send query to DNS server ' . $this->nameserver . ' (' . $errstr . ')', $errno));
        }
        $loop = $this->loop;
